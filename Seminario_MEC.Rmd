...
```{r, echo = FALSE, message=FALSE, warning=FALSE}
Depresion_final <- Depresion_final %>%
  mutate(Comunidad = rep(c('Total nacional', 'Andalucía', 'Aragón', 'Asturias', 'Balears', 'Canarias', 'Cantabria', 'Castilla y León','Castilla-La Mancha', 'Cataluña', 'Comunitat Valenciana', 'Extremadura', 'Galicia', 'Madrid', 'Murcia', 'Navarra', 'País Vasco', 'Rioja','Ceuta', 'Melilla'), each = 4, length.out = 240))
Depresion_final
```
...
EnfMentales_final<- EnfMentales_final %>%
  filter(Enfermedad_Mental %in% c(" Ansiedad crónica", " Depresión crónica", "Esquizofrenia", " Trastorno bipolar", " Trastornos del espectro del autismo"))
EnfMentales_final
```
...
```{r}
depr_y_suicidio <- right_join(Depresion_final, Suicidio_final, by = "Comunidad")
depr_y_suicidio
unique(depr_suicidio)
depr_y_suicidio <- depr_y_suicidio[, c("Comunidad","Sexo_Depresion","Prevalencia","Valor_Depresion","Valor_Suicidio")] %>%
  filter(depr_y_suicidio$Comunidad!="Total nacional")
depr_y_suicidio 
```
...
depr_y_suicidio <- depr_y_suicidio %>%
  group_by(Comunidad) %>%
  summarise(vDepresion = mean(Valor_Depresion), vSuicidio = mean(Valor_Suicidio))
```
...
```{r}
pib_Suic <- left_join(suicidio2020, pib2020, by = "Comunidad")
pib_Suic
```
