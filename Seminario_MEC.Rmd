...
<br>
-	Estudiar la relación entre la tasa de suicidio y las enfermedades mentales por comunidad autónoma.
<br>
...
```{r, echo = FALSE, message=FALSE, warning=FALSE}
EnfermedadesMentales <- fromJSON(file = "Data/EnfermedadesMentales.json")  #La importación de este dataset sigue el mismo procedimiento que el anterior.
```
...
```
```{r, echo = FALSE, message=FALSE, warning=FALSE}
levels(factor(EnfMentales_final$Comunidad))
```
```{r, echo = FALSE, message=FALSE, warning=FALSE}
EnfMentales_final <- EnfMentales_final %>%
  select(document.id, Sexo_EnfMental, Enfermedad_Mental, Valor_EnfMental)
```

```{r, echo = FALSE, message=FALSE, warning=FALSE}
EnfMentales_final <- EnfMentales_final %>%
  mutate(Comunidad = rep(c('Total nacional', 'Andalucía', 'Aragón', 'Asturias', 'Balears', 'Canarias', 'Cantabria', 'Castilla y León','Castilla-La Mancha', 'Cataluña', 'Comunitat Valenciana', 'Extremadura', 'Galicia', 'Madrid', 'Murcia', 'Navarra', 'País Vasco', 'Rioja','Ceuta', 'Melilla'), each = 15, length.out = 300))
EnfMentales_final
```
```{r, echo = FALSE, message=FALSE, warning=FALSE}
levels(factor(EnfMentales_final$Enfermedad_Mental))
EnfMentales_final<- EnfMentales_final %>%
  filter(Enfermedad_Mental %in% c(" Ansiedad crónica", " Depresión crónica", "Esquizofrenia", " Trastorno bipolar", " Trastornos del espectro del autismo"))  #Filtamos la columna Enfermedad_Mental para que aparezcan unicamnete los valores deseados.
EnfMentales_final
```
...
```{r}
#library("knitr")
#include_graphics("Seminario_MEC/Data/CapturaSeminario.png")
```
...
```{r}
# Esto no se si está bien porque no se relacionar depresion y suicidio
...
pib_Suic2020 <- pib_Suic2020 %>%
  group_by(Comunidad) %>%  #Agrupamos los datos por Comunidad.
  summarise(vSuicidio = mean(Valor_Suicidio), vPIB = mean(Total))
pib_Suic2020
```


```{r}
ggplot(pib_Suic, aes(x = Comunidad)) +
  geom_bar(aes(y = Valor_Suicidio, fill = "Suicidio"), position = "dodge", stat = "identity") +
  geom_bar(aes(y = Total, fill = "Depresion"), position = "dodge", stat = "identity") +
  labs(title = "Relación entre la depresión y Casos de Suicidio por Comunidad",
       x = "Comunidad",
       y = "Valor") +
  scale_fill_manual(values = c("Depresion" = "green", "Suicidio" = "brown")) +
  theme_minimal()
```
...
pib_suicidio <- full_join(Suicidio_final,PIB.csv, by = "Comunidad")
```
...
```{r}
#NOOOO ya esta hecho arriba (En Depresion)
Depresion_final <- Depresion_final %>%
  mutate(Comunidad = ifelse(Comunidad == 'Asturias (Principado de)', 'Asturias', Comunidad))
...
```{r}
depr_suicidio <- full_join(Depresion_sex, Suicidio_final, by = c("Comunidad"))
depr_suicidio
```


```{r}
depr_suicidiosin_na <- na.omit(depr_suicidio)
ggplot(data = depr_suicidio, aes(x=Valor_Suicidio, y = Sexo_Depresion)) +
  geom_bar(stat = "identity", fill = "chartreuse2") +
  labs(title = "Depresión vs Suicidio por sexo", x = "Sexo", y = "Valor suicidio")

```
```{r}
grafico_Suicidio_Sexo <- ggplot(depr_suicidio, aes(x = Sexo_Depresion, y = Valor_Suicidio, fill = Sexo_Depresion)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar(theta = "y") +
  labs(title = "Depresión vs Suicidio por sexo",
       fill = "Sexo_Depresion",
       x = NULL,
       y = NULL) +
  theme_minimal() +
...
print(grafico_Suicidio_Sexo)
  
```

```{r}
grafico_Suicidio_sexo <- ggplot(depr_suicidio, aes(x = Sexo_Depresion, y = 1, fill = Sexo_Depresion)) +
...
ggplot(pib_suici, aes(x = Total, y = Valor_Suicidio)) +
  ...
  labs(title = "Relación entre PIB y Casos de Suicidio por Comunidad",
       ...
       y = "Casos de Suicidio") +
  theme_minimal()
ggplot(pib_suici, aes(x = Total, y = Valor_Suicidio, label = Comunidad)) +
  geom_point(color = "darkblue", size = 1) +
  geom_text(aes(label = Comunidad)) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +  # Línea de tendencia lineal con intervalo de confianza
  labs(title = "Relación entre PIB y Casos de Suicidio por Comunidad",
       x = "PIB",
       y = "Casos de Suicidio") +
  theme_minimal() 
...
Vamos a unir el conjunto de datos de enfermedades mentales, y de suicidio, para ver la relación que existe entre estas patologias y la tasa de suicidio:

```{r, echo = FALSE, message=FALSE, warning=FALSE}
str(EnfMentales_final)
EnfMentales_final
```
...
```{r, echo = FALSE, message=FALSE, warning=FALSE}
EM_suicidio <- full_join(EnfMentales_final, Suicidio_final, by = c("Comunidad"))
EM_suicidio
```

```{r}
ggplot(data = EnfMentales_final, aes(x=Valor_EnfMental, y = Comunidad, fill =Comunidad)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Gráfico de Barras de EM", x = "Comunidad", y = "Valor") +
theme_minimal()
```

```{r}
ggplot(data = EnfMentales_final, aes(x=Enfermedad_Mental, y = Valor_EnfMental, fill =Enfermedad_Mental)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Gráfico de Barras de EM", x = "Comunidad", y = "Valor") +
theme_minimal()
```
