...
```{r}
#Separación variable nombre (tipo string)
nombre_separado <- depresionCCAA %>%
  separate(Nombre, into = c("Sexo", "Comunidad", "GradoDepresion"), sep = ",") 
nombre_separado
...
union <- merge (nombre_separado,meta,by = "document.id", all = TRUE)
union
```


```{r}
union_final <- merge(union, valor,by = "document.id", all = TRUE)
union_final
```
Seleccion de columnas 
```{r}
depr_final <- select(.data=union_final,c(document.id,GradoDepresion,Sexo,Valor,Comunidad))
View(depr_final)
```
...
```{r}
nombreMental <- enfermedadesMentales %>%
  separate(Nombre, into = c("Comunidad", "Sexo", "EnfermedadMetal"), sep = ",") 
nombreMental
```

```{r}
# Union tablas de depresion
union2 <- merge (dataM,metaM,by = "document.id", all = TRUE)
union2
```

```{r}
unionEnfMentales <- merge (union2,nombreMental,by = "document.id", all = TRUE)
unionEnfMentales
```

```{r}
#Seleccion columnas
enfMent_final <- select(.data=unionEnfMentales,c(document.id,Sexo,Valor,Comunidad,EnfermedadMetal))
enfMent_final

#Eliminar duplicados
mental <- distinct(enfMent_final)
mental
...
```{r}
pibCCAA <- pib %>%
  separate(Nombre, into = c("Comunidad", "Dato_Base", "Tasa_PIB"), sep = ".") 
View(pibCCAA)
```
...
```{r}
union_suicidio <- full_join (TasaSuicidio,data, by="document.id")
...
Union data y metadata
```{r}
s <- full_join(data,meta,by="document.id")
s
```

UNION DEPRESIÓN Y SUICIDIO
```{r}
#depr_y_suicidio <- left.join(x = depr, y=  
```
