...
for (i in seq_along(PIB)) {
  ...
  if ("Data" %in% names(PIB[[i]]) && length(PIB[[i]]$Data) > 0) {
    ...
    # Hacer algo con la información (puedes imprimir, almacenar en un data frame, etc.)
    cat("Fila:", i, "\n")
    cat("COD:", cod, "\n")
    cat("Nombre:", nombre, "\n")
    cat("Anyo:", anyo, "\n")
...
Tras probar a utilizar el set de datos "PIB" con JSON, hemos comprobado que solo aparecen los datos que nos interesan (Anyo y Valor) en la primera fila, en cambio en el resto no existen estos valores, con lo cual, no tenemos otra opción que utilizar este set de datos en .csv.

```{r,echo=FALSE}
knitr::include_graphics("C:/Users/Elena/Desktop/INGENIERÍA DE LA SALUD/CUARTO/FUENTES/SEMINARIO/Seminario_MEC/Data/CapturaSeminario.png")
...
union_suicidio
=======
### CASO 2: Depresión y Suicidio en España
...
#union_suicidio <- full_join(data,TasaSuicidio,by="document.id") %>%
  #relocate(Valor, .after = Prevalencia)
#union_suicidio
>>>>>>> e392712b1951bd702f5808f44adb0a4e680f0a0b
```
...
ggplot(data = Suicidio, aes(x=Valor_Suicidio, y = Comunidad), fill="BLUE") +
  geom_bar(stat = "identity") +
