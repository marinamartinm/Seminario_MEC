...
Podemos ver como hubo un pico en 2019, que disminuyó considerablemente al año siguiente, pero que en 2021 volvió a aumentar en un gran número de casos. Nosotros hemos deducido que esto se debe a la pandemia. Relacionandolo un poco con aquello con lo que queremos compararlo: el ocio y la vida social. Ya que creemos socializar y hacer planes fuera de la rutina aumenta las condiciones de vida de las personas, y como durante ese año, se nos impusieron ciertas normas que nos lo impedían, puede que un aumento de los casos se deba a este asunto.
```{r}
...
```{r}
union_depresion_f<- union_depresion %>%
...
```{r}
ggplot(data = union_depresion_f, aes(x=Valor, y = Prevalencia)) +
  geom_bar(stat = "identity", fill = "thistle3") +
...
```{r}
union_depresion_s<- union_depresion_f %>%
...
```{r}
ggplot(data = union_depresion_s, aes(x=Valor, y = Sexo)) +
...
```{r}
union_depresion_f2 <- union_depresion_f %>%
  filter(Comunidad!=" Total")
ggplot(data = union_depresion_f2, aes(x=Valor, y = Comunidad)) +
...
Suicidio <- 
  Suicidio %>%
  rename("Periodo" = AnoSuicidio)
```


```{r}
levels(factor(Suicidio$Periodo))
...
```
```{r}
Suicidio$Periodo <- as.double(as.character(Suicidio$Periodo))
...
```{r}
pib_suici <- left_join(PIB.csv, Suicidio, by=c("Comunidad","Periodo"))
