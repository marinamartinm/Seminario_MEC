...
```
```{r}
union2 <- merge(union,datad,by = "document.id", all = TRUE)
View(union2)
```
```{r}
union2 <- select(union2, -JSON)
```

```{r}
DEPRESION <- select(.data = union2, document.id:GradoDepresion)#, Varieble.Nombre)#, Variable.id, Valor)
Depre <- select(.data = DEPRESION, )
```
...
```{r}
# Union tablas de depresion
Union <- merge(nombreMental,metaM,by = "document.id", all = TRUE)
Union
```
...
```
```{r}
UnionS <- merge(TasaSuicidio,MetaS,by = "document.id", all = TRUE)
UnionS
```
