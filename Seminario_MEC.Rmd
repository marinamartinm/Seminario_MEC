...
El formato de datos escogido son tanto .csv como .json; aunque en un principio trabajaremos únicamente con los datos en formato JSON.
...
```
Como podemos observar, es imposible realizar el estudio con este conjunto de datos en formato Json, por ello, hemos tenido que hacerlo con csv.
```{r}
library("knitr")
...
### 3º Conjunto de datos: Tasa de Suicidio por CCAA
```{r}
...
Hay que tener en cuenta que los valores muestran el porcentaje de crecimiento y decrecimiento que sufrió cada CCAA en relación a el PIB.
...
suicidio2020 <- Suicidio_final %>% filter(Año_Suicidio==2020)
...
```
Antes de realizar el planteamineto de los resultados, me gustaria explicar porque hay 6 comunidades que no muestran los casos de suicidio, estas comunidades son Aragón, Asturias, Balears, Madrid, Murcia y Navarra. Para estas comunidades el PIB no es mayor que la media de suicidios, sino que al hacer la media de los casos de suicidio para estas comunidades devolvió NAs, en vez de un valor, y por lo tanto no aparecen dos colores como en el resto de comunidades. Para la gráfica de 2018 se suman alguna barra más con un único color, pero en esta última gráfica las comunidades con valores aunsentes siguen siendo las mismas,  aunque a la hora de ser representadas mo se muestran los casos de suicidio, como pueden ser Castilla la Mancha o Castilla y León.

Como podemos ver, tanto en las tablas, como en las gráficas obtenidas, no se aprecia una relación directa entre la Tasa de Suicidio y el Producto Interior Bruto de de cada Comunidad Autónoma, al igual que tampoco existe ninguna relación en como varian estos valores a lo largo de estos 3 años que comparamos, ya que en cada conjunto de datos o en cada gráfica, cada vez toman valores totalmente diferentes tanto para los casos de Suicidio, como del PIB, por lo tanto podemos confirmar que no existe ningina relación, ni nigún tipo de vinculo que nos ayude a explicar como el PIB de cada Comunidad Autonoma influye en el número de suicidios que se dan en cada una de ellas.  
...
ggplot(pib_suici, aes(x = Total, y = Valor_Suicidio, label = Comunidad)) +
  ...
  labs(title = "PIB y Casos de Suicidio por Comunidad",
       x = "PIB",
       y = "Casos de Suicidio") +
  theme_minimal() 
```
