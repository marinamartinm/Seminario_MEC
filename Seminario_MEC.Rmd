...
```{r}
depr_y_suicidio <- right_join(Depresion_final, suicidio2020, by = "document.id")
depr_y_suicidio
```
...
Depresion_filt
```
En esta gráfica podemos comprobar los distintos tipos de depresión: thistle3
```{r}
...
```{r}
ggplot(data = depr_suicidio, aes(x=Sexo_Depresion, y = Valor_Suicidio)) +
  geom_bar(stat = "identity", fill = "chartreuse2") +
...
```{r}
ggplot(data = EnfMentales_final, aes(x=Comunidad, y = Valor_EnfMental, fill =Comunidad)) +
  geom_bar(stat = "identity", position = "dodge") +
